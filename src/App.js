...
class App extends Component {
  ...
  readText=()=>{
    const synth = window.speechSynthesis;
    const utterance = new SpeechSynthesisUtterance(`${this.state.text} detected`);
    synth.speak(utterance);
    this.setState({text:""});
    this.speak=(object) => {
      if(SpeakNumber==150){
  ...
  readTime=()=>{
    ...
    const synth = window.speechSynthesis;
    const utterance = new SpeechSynthesisUtterance(`The time is ${strTime}`);
    synth.speak(utterance);
    this.setState({text:""});
    this.speak=(object) => {
      if(SpeakNumber==150){
        ...
        <div className="InnerBoxContainer">
          <div className="MainWork">
            ...
            <div className="textBoxContainer">
              ...
              {text!=="loading Text" && text!==""?this.readText():""}
              {text!==""?(
                <p>{text}</p>
              ):""}
              {classes.length?(
